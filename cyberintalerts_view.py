...
def get_enriched_alerts_view(request, **kwargs):
    ...
    if phantom.is_fail(ret_val):
        return HttpResponse(
            f"Failed to get alerts: {action_result.get_message()}", status=500
        )
    ...
    if phantom.is_fail(ret_val):
        return HttpResponse(
            f"Failed to get full alerts: {action_result.get_message()}", status=500
        )

    ret_val, enriched_alerts = connector._enrich_alert_indicators(
        full_alerts, action_result
    )
    if phantom.is_fail(ret_val):
        return HttpResponse(
            f"Failed to enrich alerts: {action_result.get_message()}", status=500
        )
...
def submit_takedown_view(request, **kwargs):
    ...
    if request.method == "POST":
        ...
        if phantom.is_fail(result):
            return HttpResponse(
                f"Failed to submit takedown: {action_result.get_message()}", status=500
            )
...
def takedown_view(request, **kwargs):
    ...
    if not customer_id:
        return HttpResponse(
            "Error: customer_id query parameter is required.", status=400
        )
